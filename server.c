	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
                case 'G': // Cas guilty
					...
					int playerIdx, cardIdx;
					sscanf(buffer, "%c %d %d", &com, &playerIdx, &cardIdx);
					
					if (cardIdx == deck[12]) { // Vérification de l'exactitude
						sprintf(reply, "%s gagne. %s accusé correctement. !\n", tcpClients[playerIdx].name, nomcartes[cardIdx]);
						broadcastMessage(reply);
					} else {
						joueurCourant = (joueurCourant + 1) % 4; // Passage au tour suivant
						sprintf(reply, "M %d", joueurCourant);
						broadcastMessage(reply);
					}
					break;
				...
				case 'O': // Cas Observation
					// Gestion des observations générales				
					int playerID; 
					int symbolID;
					sscanf(buffer, "%c %d %d", &com, &playerID, &symbolID);
					
					for (i = 0; i < 4; i++) { 
						if (i != playerID) {
							if (tableCartes[i][symbolID] == 0) { // Vérification des caractéristiques
								sprintf(reply, "V %d %d %d", i, symbolID, 0);
								broadcastMessage(reply);
							}
						}
					}
				
					joueurCourant = (joueurCourant + 1) % 4; // Actualisation du tour
					sprintf(reply, "M %d", joueurCourant);
					broadcastMessage(reply);
					break;
				...
				case 'S': // Cas Spécifié 
					...
					int target; 
					int attribute;
					sscanf(buffer, "%c %d %d %d", &com, &requestor, &target, &attribute);
					
					sprintf(reply, "V %d %d %d", target, attribute, tableCartes[target][attribute]); // Transmission des informations 
					broadcastMessage(reply);
					
					joueurCourant = (joueurCourant + 1) % 4; // Mise à jour du tour
					sprintf(reply, "M %d", joueurCourant);
					broadcastMessage(reply);
					break;
