	...
	if (SDL_PollEvent(&event))
	{
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				if ((mx<200) && (my<50) && (connectEnabled==1))
				{
					...
					// Envoi de la requête d'initiation au serveur central
				...
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					// Transmission de l'hypothèse de culpabilité
					sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
					...
					// Transmission de la demande d'observation générale
					sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
					...
					// Transmission de la requête d'information ciblée
					sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
        ...
        if (synchro==1)
        {
			...
			switch (gbuffer[0])
			{
				...
				case 'I':
					// Récupération et stockage de l'identifiant attribué
					sscanf(gbuffer + 2, "%d", &gId);
					printf("Identifiant assigné: %d\n", gId);
					break;

				// Message 'L' : le joueur recoit la liste des joueurs
				case 'L':
					// Mise à jour du registre des participants
					sscanf(gbuffer + 2, "%s %s %s %s", 
					gNames[0], gNames[1], gNames[2], gNames[3]);
					printf("Registre des enquêteurs actualisé\n");
					break;
				// Message 'D' : le joueur recoit ses trois cartes
				case 'D':
					// Acquisition des indices personnels
					sscanf(gbuffer + 2, "%d %d %d", &b[0], &b[1], &b[2]);
					initCards(); 
					printf("Indices attribués: %d, %d, %d\n", b[0], b[1], b[2]);
					break;

				// Message 'M' : le joueur recoit le n° du joueur courant
				// Cela permet d'affecter goEnabled pour autoriser l'affichage du bouton go
				case 'M':
					// Détermination du tour de jeu actuel
					int currentPlayer;
					sscanf(gbuffer + 2, "%d", &currentPlayer);
					
					if (currentPlayer == gId) {
						goEnabled = 1;
						printf("Votre tour d'enquête commence\n");
					} else {
						goEnabled = 0;
						printf("Enquêteur %d en action\n", currentPlayer);
					}
					break;
				...
				case 'V':
					...
					int investigator, clueType, quantity;
					sscanf(gbuffer + 2, "%d %d %d", &investigator, &clueType, &quantity);
					
					if (tableCartes[investigator][clueType] == -1) { // Mise à jour conditionnelle
						tableCartes[investigator][clueType] = quantity; // Enregistrement de l'information
						printf("Information découverte: Enquêteur %d, Indice %d, Quantité %d\n", 
							investigator, clueType, quantity);
					}
					break;
			}
